to setup
  ...
  ask patches  ;; randomly set the patches' colors
    [ set pcolor (random kleuren) * 10 + 5
...
to go
  if (variance [pcolor] of patches) = 0 [
    if user-yes-or-no? "Er zal niets meer veranderen. Een nieuwe simulatie starten?" [ setup ]
  ]
  ask patches [
    ;; each patch randomly picks a neighboring patch
    ;; to copy a color from
    if any? neighbors with [pcolor != 0] [
      set pcolor [pcolor] of one-of neighbors with [pcolor != 0]
    ]
  ]
  tick
  if (variance [pcolor] of patches) = 0 [stop]
end

to kill
  ask patches [
    if pxcor < overstroming - 20 [
      set pcolor 0
    ]
  ]
end

to-report no-pcolors
  report length remove-duplicates [pcolor] of patches
end

; Copyright 1997 Uri Wilensky.
; See Info tab for full copyright and license.N
@#$#@#$#@
GRAPHICS-WINDOW
260
10
...
359
Uitgestorven
kleuren + ifelse-value (any? patches with [pcolor = 0]) [1] [0] - length remove-duplicates [pcolor] of patches
0
1
11
